...
export class StillumPortalBridge implements Disposable {
    ...
    /**
     * Save files by delegating to the portal via postMessage (avoids CORS).
     * Sends a save-request with a files map, waits for the portal's save-response.
     */
    async saveFiles(artifactId: string, versionId: string, files: Record<string, string>): Promise<void> {
